...
src_prepare() {
    ...
    # allow single click in file dialogs
	epatch "${FILESDIR}/${P}-singleclick.patch"
...
pkg_postinst() {
	...
	elog "If dconf-editor does not list	org/gtk/settings/file-chooser/click-policy"
	elog "run the following command as user to set the policy to single-click:"
	elog "dconf write \"/org/gtk/settings/file-chooser/click-policy\" \"'single'\""
}
